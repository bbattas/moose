##############################################################################
# File: 02_inc_kernel_action.i
# File Location: /examples/agg/01_initial_testing/30_newCode/02_inc_kernel_action
...
# -----
# Description:
#  Testing the new kernel action for inclination kernels
#  var name base and op num are being specified in the action as theyre inputs
#   in the kernel, in that req coupled vars thing, so all is good with that
#
##############################################################################
...
[UserObjects]
  [voronoi]
    ...
    file_name = '../../26_polycrystal/12_num_grains/00_sub/d3d_2D_5gr.txt'
    int_width = 8
  []
  [grain_tracker]
    type = GrainTracker
...
[Problem]
  ...
  dump_path = Kernels/PolycrystalInclinationKernel
  # dump_path = Kernels/PolycrystalKernel
[]
...
[Materials]
  ...
  [hgb_a]
    ...
    expression = 'hb:=gr0*gr0 + gr1*gr1 + gr2*gr2 + gr3*gr3 + gr4*gr4;
                  4 * (1 - hb) * (1 - hb)' # + gr5*gr5 + gr6*gr6 + gr7*gr7 + gr8*gr8 + gr9*gr9 + gr10*gr10 + gr11*gr11;
    outputs = 'exodus'
  []
  [hgb_b]

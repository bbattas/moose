##############################################################################
# File: 03_incmat_noelemround.i
# File Location: /examples/agg/01_initial_testing/30_newCode/03_incmat_noelemround
...
# -----
# Description:
#  Checking if my way of not needing the gamma elemental rounding for when
#   using the cap/limit method instead of zeroing on tolerances
#
#
##############################################################################
...
[Materials]
  ...
  [gamma_check]
    type = ParsedMaterial
    property_name = gamma_check
    material_property_names = 'gamma_asymm'
    expression = 'gamma_asymm'
    outputs = 'exodus'
  []
  [hgb]

##############################################################################
# File: 06_custom_recomb.i
# File Location: /examples/sintering/paper3/01_test_inputs/18_custom_irrad/06_custom_recomb
...
# -----
# Description:
#  Using custom recombination material instead of DPMs
#
#
#
##############################################################################
...
[Kernels]
  ...
  [recombination_vac]
    type = MatReaction
    variable = cvac_var
    mob_name = cvi_recomb #rho_recomb
    args = 'wvac wint cint_var phi' # gr2 gr3 gr4 gr5 gr6 gr7' # grain ops mean i should include them in combined_rho
  ...
  [recombination_int]
    ...
    variable = cint_var
    mob_name = cvi_recomb #rho_recomb
    args = 'wvac wint cvac_var phi' # gr2 gr3 gr4 gr5 gr6 gr7' # grain ops mean i should include them in combined_rho
...
[Materials]
  ...
  [net_defect]
    ...
    material_property_names = 'c_gen rho_recomb'
    expression = 'c_gen + rho_recomb'
    outputs = nemesis
  []
  # IRRADIATION
  [c_gen]
  ...
  [rho_recomb] #This one is off on GB?
    ...
    coupled_variables = 'wvac wint phi cvac_var cint_var' # gr0 gr1' #gr0 gr1 gr2
    derivative_order = 1
    # additional_derivative_symbols = w # combined_rho_vac combined_rho_int
    material_property_names = 'Va a_r(phi) hv' # combined_rho_vac(wvac,phi) combined_rho_int(wint,phi) hv'
    expression = 'out:=a_r * cvac_var * cint_var / Va;
  ...
  [cvirec_new]
    type = DerivativeParsedMaterial
    property_name = cvrec_new
    coupled_variables = 'wvac wint phi cvac_var cint_var'
    derivative_order = 1
    material_property_names = 'cvi_recomb(phi,wvac,wint,cvac_var,cint_var)'

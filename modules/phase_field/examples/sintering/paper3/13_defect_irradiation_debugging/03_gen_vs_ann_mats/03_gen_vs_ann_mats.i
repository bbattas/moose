##############################################################################
# File: 03_gen_vs_ann_mats.i
# File Location: /examples/sintering/paper3/13_defect_irradiation_debugging/03_gen_vs_ann_mats
# Created Date: Wednesday February 14th 2024
# Author: Brandon Battas (bbattas@ufl.edu)
# -----
# Last Modified: Wednesday February 14th 2024
# Modified By: Brandon Battas
# -----
# Description:
#  The first tests had what seems like the vac vs int gen/ann being uneven
#  This is breaking the components down to see what they do, to try to find why
#   we dont seem to be creating equal numbers of vacancies adn interstitials?
#  Changing things in the materials to test stuff
##############################################################################
...
[Materials]
  ...
  [v_rho_vac]
    type = DerivativeParsedMaterial
    property_name = v_rho_vac
    material_property_names = 'rhov rhos hv(phi) hs'
    expression = 'hv*rhov'
    outputs = nemesis #'nemesis'
  []
  [s_rho_vac]
    type = DerivativeParsedMaterial
    property_name = s_rho_vac
    material_property_names = 'rhov rhos hv(phi) hs'
    expression = 'hs*rhos'
    outputs = nemesis #'nemesis'
  []
  [gen_combined]
    type = ParsedMaterial
    property_name = gen_combined
    material_property_names = 'a_r rho_gen_int rho_gen_vac hs'
    expression = 'rho_gen_vac - (hs * rho_gen_int)'
    outputs = 'nemesis'
  []
  [ann_combined]
    type = ParsedMaterial
    property_name = ann_combined
    material_property_names = 'rho_v_recombRate rho_i_dpm_hs a_r hs'
    postprocessor_names = 'average_rho_vac'
    expression = 'rho_v_recombRate - (a_r * rho_i_dpm_hs * average_rho_vac)'
    outputs = 'nemesis'
  []
  [ann_combined_actual]
    type = ParsedMaterial
    property_name = ann_combined_actual
    material_property_names = 'rho_v_recombRate rho_i_dpm_hs a_r hs combined_rho_vac'
    postprocessor_names = 'average_rho_vac'
    expression = 'rho_v_recombRate - (a_r * rho_i_dpm_hs * combined_rho_vac)'
    outputs = 'nemesis'
  []
  ...
  [rho_i_dpm_hs]
    ...
    # derivative_order = 0 #2
    # coupled_variables = ''
    material_property_names = 'rho_gen_int a_r hs combined_rho_vac'
    postprocessor_names = 'average_rho_vac'
    # expression = 'if(average_rho_vac!=0.0,(rho_gen_int * hs / (a_r * combined_rho_vac)),0.0)'
    expression = 'rho_gen_int * hs / (a_r * combined_rho_vac)'
    outputs = nemesis #'nemesis'
  []
  # Vacancies
  [rho_gen_vac]
  ...
  [rho_v_recombRate]
    ...
    material_property_names = 'a_r rho_i_dpm_hs combined_rho_vac'
    postprocessor_names = 'total_rhoi' # average_rho_vac'
    expression = 'a_r * rho_i_dpm_hs * combined_rho_vac'
    outputs = 'nemesis'
  []
  [rho_v_mixing]
    type = DerivativeParsedMaterial

##############################################################################
# File: 14_sk_large_noGen_massCheck.i
# File Location: /examples/sintering/paper3/13_defect_irradiation_debugging/14_sk_large_noGen_massCheck
...
# -----
# Description:
#  Testing in the large case some extra cases just to have the data when
#   we hopefully meet with mike/larry/david about how to do things
#  Checking for mass loss with the large case if there is no generation
#   of defects (but leaving the annihilation) and no mix (doing this by setting
#   the f_dot to 0.0 but leaving the kernels turned on)
#  Also set the void FFC to use the 0.5 thresholds to match the grain one for VPP
##############################################################################

##############################################################################
# File: 16_sk_large_externalPoreOnly_withIrrad.i
# File Location: /examples/sintering/paper3/13_defect_irradiation_debugging/16_sk_large_externalPoreOnly_withIrrad
...
#  Running the same structure (large 2d) but with only the external pore and
#   WITH the fission enabled (1e-9 fdot, so mix and gen are >0)
##############################################################################
...
[Mesh]
  [ebsd_mesh]
    ...
    filename = ../00_d3d_txt/2D_100x100um_8umavg_externalOnly.txt
  []
  # [subdomain_external]

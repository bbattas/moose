##############################################################################
# File: 05_small2D_equalIntWidths.i
# File Location: /examples/sintering/paper3/15_newParams_2D_noIrradiation/05_small2D_moreTests
...
#   Lower surface D (calculated in GPIsoMat) (from Ali's paper)
#   New GBMobility Values (Tonks/PC/Jake GG Paper)
#   Temp dependent gb/surface energy (Tonks/PC/Jake GG Paper)
#  This one is looking at if gb and surface width in GPIso are equal to look at D
##############################################################################
...
[Functions]
  ...
  [gb_e_func]
    ...
    symbol_names = 'T'
    symbol_values = 'f_T'
    expression = '6.24151*(-5.87e-4*T + 1.56)'
  []
  [surf_e_func]
    type = ParsedFunction
    symbol_names = 'gb_e_func'
    symbol_values = 'gb_e_func'
    expression = '2*gb_e_func'
  []
  # [rhoi_func]
  #   type = ParsedFunction

##############################################################################
# File: 14_noIrrKernels_debug.i
# File Location: /examples/sintering/paper3/16_newIrradiation_testing/14_noIrrKernels_debug
# Created Date: Monday April 29th 2024
# Author: Brandon Battas (bbattas@ufl.edu)
# -----
# Last Modified: Monday April 29th 2024
# Modified By: Brandon Battas
# -----
# Description:
#  A debugging input to see why the input isnt converging at all even with the
#   fission kernels turned off?
#  Problem is with the interstitial concentration
#
##############################################################################
...
[Materials]
  ...
  [k_constants]
    ...
    prop_values = '26.8 26.8 158 158' #3.6e21 3.6e21' #'26.8 26.8 3.6e21 3.6e21'#154.16 154.16
  []
  # [ksu]
  #   type = ParsedMaterial
  #   property_name = ksu
  ...
  [cv_eq]
    ...
    constant_expressions = '5.542e-12 5.679e-09' #Irradiation
    # constant_expressions = '5.542e-12 1.683e-03' #No Irradiation
  ...
  [ci_eq]
    ...
    constant_names = 'cb cgb'
    constant_expressions = '1.050e-29 5.878e-04' #Irradiation
    # constant_expressions = '1.050e-29 5.043e-12' #No Irradiation
    expression = 'lam:=gr0^2 + gr1^2 + gr2^2 + phi^2;
                  cb + 4 * (cgb - cb) * (1-lam)^2'
    outputs = none #'nemesis'
  []
  ...
  [f_phase]
    type = ParsedMaterial
    property_name = f_phase
    coupled_variables = 'wvac wint phi'
    material_property_names = 'hs(phi) hv(phi) omegas rhosu rhosi omegav rhovu rhovi'
    expression = 'hs * (omegas + rhosu * wvac + rhosi * wint) + hv * (omegav + rhovu * wvac + rhovi * wint)'
    outputs = nemesis
  []
  [omega_tot]
    type = ParsedMaterial
    property_name = omega_tot
    coupled_variables = 'wvac wint phi'
    material_property_names = 'hs(phi) hv(phi) omegas omegav'

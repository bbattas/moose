##############################################################################
# File: 08_2D_5pore_fr1e-8_hshsCombined.i
# File Location: /examples/sintering/paper3/18_newCeq/08_2D_5pore_fr1e-8_hshsCombined
...
# -----
# Description:
#  Weird just to test, using hs and 1-hs for combined rhovac and rhoint
#  Was hv and 1-hv before but phi gen is weird, and 07 is hs hv so this
#   is just to compare
#
##############################################################################
...
[Mesh]
  [ebsd_mesh]
    ...
    filename = ../00_d3d_txt/2D_50x50um_8umavg_5pore.txt
  []
  [subdomain_external]
...
[Materials]
  ...
  [combined_rho_vac]
    ...
    derivative_order = 2
    material_property_names = 'rhovu(wvac) rhosu(wvac) hs(phi)'
    expression = 'hs*rhosu + (1-hs)*rhovu' #'(1-hv)*rhos' #
  ...
  [combined_rho_int]
    ...
    derivative_order = 2
    material_property_names = 'rhovi(wint) rhosi(wint) hs(phi)'
    expression = 'hs*rhosi + (1-hs)*rhovi' #'(1-hv)*rhos' #
    outputs = nemesis #'nemesis'
  []

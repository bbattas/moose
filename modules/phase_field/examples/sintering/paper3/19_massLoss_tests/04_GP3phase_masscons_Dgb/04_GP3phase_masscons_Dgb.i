##############################################################################
# File: 04_GP3phase_masscons_Dgb.i
# File Location: /examples/sintering/paper3/19_massLoss_tests/04_GP3phase_masscons_Dgb
...
# -----
# Description:
# Testing a variation using different D on gb vs bulk but still keeping ceq constant
#
...
[Materials]
  ...
  [rhod]
    ...
    derivative_order = 2
  []
  [const] #renamed constant value ceqs to _c
    type = GenericConstantMaterial
    prop_names = 'kappa_c  kappa   L   D_c    Vm   ka    caeq kb    cbeq  kd    cdeq  gab gad gbd  mu  tgrad_corr_mult'
  ...
  [D]
    type = DerivativeParsedMaterial
    property_name = D
    constant_names = 'Db Dgb'
    constant_expressions = '1 10'
    coupled_variables = 'etaa0 etab0 etad0'
    material_property_names = 'hgb(etaa0,etab0,etad0)'
    expression = 'hgb * Dgb + (1 - hgb) * Db'
  []
  [cmat]
    type = ParsedMaterial
    property_name = cmat
    coupled_variables = 'w etaa0 etab0 etad0'
    material_property_names = 'Vm rhoa(w) rhob(w) rhod(w) ha(etaa0,etab0,etad0) hb(etaa0,etab0,etad0) hd(etaa0,etab0,etad0)'

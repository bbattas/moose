##############################################################################
# File: 05_noCons_gbCeq_adaptive.i
# File Location: /examples/sintering/paper3/19_massLoss_tests/13_2grain_noVoid_massconsTests/05_noCons_gbCeq_adaptive
...
# -----
# Description:
#  Testing mesh and timestepping effects on mass conservation
#  This is without conservation and using GB/B dependent ceq
#  UR: 0 = ~11k, 1 = ~44k, 2 = ~175k
#  This is checking an adaptive timestepper
##############################################################################
...
[Kernels]
  ...
  [Diffusion]
    ...
    variable = w
    diffusivity = Dchi
    args = 'etaa0 etab0' #chi is technically a function of these if Im overspecifying
  []
  [coupled_etaa0dot]
...
[Materials]
  ...
  [Mobility]
    type = DerivativeParsedMaterial
    property_name = Dchi
    material_property_names = 'D chi(etaa0,etab0)'
    coupled_variables = 'etaa0 etab0'
    expression = 'D*chi'
    derivative_order = 2
  []
...
[Outputs]
  ...
  exodus = false
  checkpoint = false
  # file_base = 02_noCons_gbCeq_base
[]

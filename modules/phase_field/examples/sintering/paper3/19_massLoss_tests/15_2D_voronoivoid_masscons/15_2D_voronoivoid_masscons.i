##############################################################################
# File: 15_2D_voronoivoid_masscons.i
# File Location: /examples/sintering/paper3/19_massLoss_tests/15_2D_voronoivoid_masscons
...
# -----
# Description:
#  Testing using voronoivoidIC to see if thats contributing to the issue with
#   timestep staying very very small (0.0005) and the solve being so slow
#  Vacancy only right now?
#
##############################################################################
...
[Materials]
  ...
  [cv_eq]
    ...
    property_name = cv_eq
    coupled_variables = 'gr0 gr1 gr2 gr3 phi T wvac'
    material_property_names = 'hgb(phi,gr0,gr1,gr2,gr3)' #'rhovi(vac) rhosi hv(phi)'
  ...
  [ci_eq]
    type = DerivativeParsedMaterial
    property_name = ci_eq
    coupled_variables = 'gr0 gr1 gr2 gr3 phi T wint'
    material_property_names = 'hgb(phi,gr0,gr1,gr2,gr3)' # 'rhovi(wint) rhosi(wint) hv(phi)'
    constant_names = 'cb cgb'
  ...
  [combined_rho_vac]
    ...
    coupled_variables = 'wvac phi gr0 gr1 gr2 gr3'
    derivative_order = 2
    material_property_names = 'rhovu(wvac,gr0,gr1,gr2,gr3) rhosu(wvac,gr0,gr1,gr2,gr3) hv(phi) hs(phi)'
  ...
  #   derivative_order = 2
  #   material_property_names = 'rhovi(wint,gr0,gr1,gr2,gr3) rhosi(wint,gr0,gr1,gr2,gr3) hv(phi) hs(phi)'
  ...
  #   material_property_names = 'a_r(phi,gr0,gr1,gr2,gr3) combined_rho_vac(wvac,phi,gr0,gr1,gr2,gr3) combined_rho_int(wint,phi,gr0,gr1,gr2,gr3)'
  ...
  [hgb]
    ...
    expression = '16 * ( (gr0 * gr1)^2 + (gr0 * gr2)^2 + (gr0 * gr3)^2 +
    (gr1 * gr2)^2 + (gr1 * gr3)^2 + (gr2 * gr3)^2)'
    outputs = none
  []
  # MASS CONSERVATION
  [hoverk_vu]
  ...
  [cvueq_mask] # We assume vacancy concentration in void phase = 1.0
    ...
    coupled_variables = 'phi gr0 gr1 gr2 gr3'
    material_property_names = 'hv(phi)' # cv_eq(phi,gr0,gr1,gr2,gr3)'
  ...
  [csueq_mask]
    ...
    coupled_variables = 'phi gr0 gr1 gr2 gr3'
    material_property_names = 'hs(phi) cv_eq(phi,gr0,gr1,gr2,gr3)'
    expression = 'hs * cv_eq'
  []
  ...
  #   coupled_variables = 'phi gr0 gr1 gr2'
  #   material_property_names = 'hv(phi)' # ci_eq(phi,gr0,gr1,gr2,gr3)'
  ...
  #   coupled_variables = 'phi gr0 gr1 gr2'
  #   material_property_names = 'hs(phi) ci_eq(phi,gr0,gr1,gr2,gr3)'
  #   expression = 'hs * ci_eq'
  # []
[]
...
[Postprocessors]
  ...
  [grain_tracker_fc]
    type = FeatureFloodCount
    variable = 'gr0 gr1 gr2 gr3' # gr3 gr4 gr5 gr6 gr7 gr8 gr9' #'gr0 gr1 gr2'
    threshold = 0.5 #0.2

##############################################################################
# File: 25_2grain_withVoid_fullFission.i
# File Location: /examples/sintering/paper3/19_massLoss_tests/25_2grain_withVoid_fullFission
# Created Date: Wednesday June 12th 2024
# Author: Brandon Battas (bbattas@ufl.edu)
# -----
# Last Modified: Thursday June 13th 2024
# Modified By: Brandon Battas
# -----
# Description:
#  Testing input 22 mass conservtion when i add irradiation kernels to start
#   changing the defect densities
#  This input is using all the irradiation values (ceq and k)! unlike 24 which
#   used the non-irradiation version
#  Right now have to reduce recombination to 1/100 x the old version or it wont
#   start solving past the first step (just too big compared to IC)
#  NOPE: Once i switched the irr kernels onto the c variables instead of chempot
#   it solved fine with full R
#  With irraiation on the C variables need to multiply all those materials by Va
##############################################################################
...
[Functions]
  ...
  [ic_func_cvGB]
    ...
    symbol_values = '2000 -5000 7500 15000 3.877e-04 4.347e-03'
  ...
  [ic_func_ciGB]
    type = ParsedFunction
    symbol_names = 'iw x0 y0 r cb cgb'
    symbol_values = '2000 -5000 7500 15000 7.258e-09 5.900e-06'
    expression = 'd:=sqrt((x-x0)^2+(y-y0)^2);
    cb + (cgb - cb)*16*((1-0.5*(1.0-tanh((r-d)/iw))) * (0.5*(1.0-tanh((r-d)/iw))))^2'
  []
[]
...
[Materials]
  ...
  [k_constants]
    ...
    prop_values = '7.751e2 7.751e2 5.711e5 5.711e5' #'6.569e2 6.569e2 5.461e7 5.461e7'
  []
  [gb_e_mat] # eV/nm^2
    type = ParsedMaterial
  ...
  [rho_recomb]
    ...
    material_property_names = 'Va a_r(phi,gr0,gr1) combined_rho_vac(wvac,phi,gr0,gr1) combined_rho_int(wint,phi,gr0,gr1)'

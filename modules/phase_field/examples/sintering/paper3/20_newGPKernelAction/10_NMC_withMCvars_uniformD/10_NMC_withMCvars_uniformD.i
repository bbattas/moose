##############################################################################
# File: 10_NMC_withMCvars_uniformD.i
# File Location: /examples/sintering/paper3/20_newGPKernelAction/10_NMC_withMCvars_uniformD
...
# -----
# Description:
#  Interstitials all the same as vacancies except ceq=0 in the pore instead of 1
#  Using NMC solve for chempot but keeping the cv/ci-var and their kernels to
#   see how those evolve compared to the material version without making chempot
#   a function of the cvars?
#
#  Testing with a constant uniform D so that chiuD = bulk value everywhere
#  NOPE: nothing happened that was just too low, ill use gb value of 1.5e3 ?
#
...
[Kernels]
  ...
  [MatDif_ci]
    ...
    diffusivity = chiuDalt #chiuD #chiiD
    args = 'phi gr0 gr1 wvac wint'
  []
  ...
  [ChiDt_wvac]
    ...
    variable = wvac
    f_name = chiu
    coupled_variables = 'phi gr0 gr1 wvac wint' #wint #not sure it needs wint since theyre independent?
  []
  [MatDif_wvac]
    type = MatDiffusion
    variable = wvac
    diffusivity = chiuDalt
    args = 'phi gr0 gr1 wvac wint' #wint#not sure it needs wint since theyre independent?
  []
  # CoupledSwitchingTimeDerivitives
  [Coupled_wvac_phi]
    type = CoupledSwitchingTimeDerivative
    variable = wvac
    v = 'phi'
    coupled_variables = 'phi gr0 gr1 wvac' #wint
    hj_names = 'hv hs'
    Fj_names = 'rhovu rhosu'
  []
  [Coupled_wvac_gr0]
    type = CoupledSwitchingTimeDerivative
    variable = wvac
  ...
  [Coupled_wvac_gr1]
    ...
    Fj_names = 'rhovu rhosu'
  []
  # INTERSTITIAL Chemical Potential
  [ChiDt_wint]
    type = SusceptibilityTimeDerivative
    variable = wint
    f_name = chiu #USING VACANCY VALUE STILL
    coupled_variables = 'phi gr0 gr1 wvac wint' #wint #not sure it needs wint since theyre independent?
  []
  [MatDif_wint]
    type = MatDiffusion
    variable = wint
    diffusivity = chiuDalt #USING VACANCY VALUE STILL
    args = 'phi gr0 gr1 wvac wint' #wint#not sure it needs wint since theyre independent?
  []
  # CoupledSwitchingTimeDerivitives
  [Coupled_wint_phi]
    ...
    variable = wint
    v = 'phi'
    coupled_variables = 'phi gr0 gr1 wint' #wint
    hj_names = 'hv hs'
    Fj_names = 'rhovi rhosi'
  []
  [Coupled_wint_gr0]
    type = CoupledSwitchingTimeDerivative
    variable = wint

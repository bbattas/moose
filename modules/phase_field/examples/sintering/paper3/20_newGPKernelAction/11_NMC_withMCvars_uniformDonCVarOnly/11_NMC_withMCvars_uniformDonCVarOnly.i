##############################################################################
# File: 11_NMC_withMCvars_uniformDonCVarOnly.i
# File Location: /examples/sintering/paper3/20_newGPKernelAction/11_NMC_withMCvars_uniformDonCVarOnly
...
[Kernels]
  ...
  [MatDif_wint]
    type = MatDiffusion
    variable = wint
    diffusivity = chiuD #USING VACANCY VALUE STILL
    args = 'phi gr0 gr1 wvac wint' #wint#not sure it needs wint since theyre independent?
  []
  # CoupledSwitchingTimeDerivitives
  [Coupled_wint_phi]

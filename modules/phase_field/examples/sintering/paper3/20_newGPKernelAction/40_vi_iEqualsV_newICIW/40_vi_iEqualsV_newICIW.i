##############################################################################
# File: 40_vi_iEqualsV_newICIW.i
# File Location: /examples/sintering/paper3/20_newGPKernelAction/40_vi_iEqualsV_newICIW
# Created Date: Friday July 26th 2024
# Author: Brandon Battas (bbattas@ufl.edu)
# -----
# Last Modified: Friday July 26th 2024
# Modified By: Brandon Battas
# -----
# Description:
#  Using interstitials and vacancies, but with GPMultiSinteringMat changed so that
#   cvi_eq = 1 instead of 0, so that all the interstitial parameters are teh same
#   as the vacancy ones, using the smaller IC IW in the circle/oval
#
##############################################################################
...
[Materials]
  ...
  [chiuD]
    ...
    bulkindex = 1
    gbindex = 1e8#-1 # -1 sets the GB D to the LANL MD Value in GPIsoMat
    surfindex = 1e9 #-1 #1e11
    vaporindex = 1
    GBwidth = 1.0
  ...
  [chiiD]
    ...
    c = phi
    T = T
    D0 = 4.2488e11 #4.0767e11 #1e13 #Ian's irradiation paper (Matzke 1987)
    Em = 4.23317 #4.08453089 #2 #Ian's irradiation paper (Matzke 1987)
    bulkindex = 1
    gbindex = 1e8#-1 #10 # -1 sets the GB D to the LANL MD Value in GPIsoMat
    surfindex = 1e9#-1 #100 #1e11
    vaporindex = 1
    GBwidth = 1.0
    surf_thickness = 1.0 #0.5
    iw_scaling = true
  ...
  [cvieq_mask] # cvieq_mask = hv*0
    ...
    derivative_order = 2
    material_property_names = 'hv(phi)'
    expression = 'hv' #0 not 1 normally
    # outputs = exodus
  []
  [csieq_mask]
    type = DerivativeParsedMaterial
...
[Outputs]
  ...
  exodus = true
  checkpoint = false
  # file_base = IW_1000_CvIC_${cvic}_1e6s
[]

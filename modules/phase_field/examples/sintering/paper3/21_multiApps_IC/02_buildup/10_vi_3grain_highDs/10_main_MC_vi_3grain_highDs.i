##############################################################################
# File: 10_main_MC_vi_3grain_highDs.i
...
# -----
# Description:
#  09 wasnt shrinking or moving pore at all? like GB moved all the way to the
#   pore but the pore didnt interact with the external void???
#  Increasing Ds here to see if that fixes it? (gb is about 1e8x so using 1e11x
#   for surface D here)
##############################################################################
...
[MultiApps]
  [full_NMC]
    ...
    execute_on = initial
    positions = '0 0 0'
    input_files = 10_sub_NMC_vi_3grain_highDs_1step.i
  []
[]

##############################################################################
# File: 11_sub_NMC_vi_3grain_highDs_largeExternal_1step.i
# File Location: /examples/sintering/paper3/21_multiApps_IC/02_buildup/11_vi_3grain_highDs_largeExternal
# Created Date: Sunday August 11th 2024
# Author: Brandon Battas (bbattas@ufl.edu)
# -----
# Last Modified: Sunday August 11th 2024
# Modified By: Brandon Battas
# -----
# Description:
#  Even with high Ds the pores didnt really change size? so like im going to try
#   making the external region much larger? they were within about 1% of each
#   other in areas (internal and external), so maybe like GG if i make external
#   much larger it will push evacuation better?
##############################################################################
...
[Mesh]
  ...
  #   combinatorial_geometry = 'x > 40000' #'y < 7500'
  #   block_id = 2
...
[ICs]
  ...
  [gr1_IC_L]
    type = FunctionIC
    variable = gr1
    function = ic_func_gr1L
    block = '0'
  []
  [gr2_IC_L]
    type = FunctionIC
    variable = gr2
    function = ic_func_gr2L
    block = '0'
  []
  [gr1_IC_R]
    type = FunctionIC
    variable = gr1
    function = ic_func_gr1R
    block = '1'
  []
  [gr2_IC_R]
    type = FunctionIC
    variable = gr2
    function = ic_func_gr2R
    block = '1'
  []
  [phi_IC]
    type = FunctionIC
...
[Functions]
  ...
  [ic_func_pore] # Right side pore
    ...
    1-0.5*(1.0-tanh((r-d)/iw)) + 0.5*(1.0+tanh((x-45000)/iw))'
  []
  [ic_func_gr1R] # Top Right grain- Right section
    ...
    symbol_values = '1000 30000 7500 5000'
    expression = 'd:=sqrt((x-x0)^2+(y-y0)^2);
    0.5*(1.0-tanh((r-d)/iw))* 0.5*(1.0+tanh((y-y0)/iw)) * 0.5*(1.0-tanh((x-45000)/iw))'
  []

##############################################################################
# File: 00_sub_NMC_1step.i
# File Location: /examples/sintering/paper3/21_multiApps_IC/03_noGB_ripening/00_sub
...
# -----
# Description:
#  1 timestep subfile for IC generation for the new no GB 1 pore input
#  External pore is about 2.5x the area of the pore with a 5000 nm gap (= r)
#
#
##############################################################################
...
[Functions]
  [ic_func_pore] # Right side pore
    ...
    symbol_values = '1000 10000 10000 5000 20000'
    expression = 'd:=sqrt((x-x0)^2+(y-y0)^2);
    1-0.5*(1.0-tanh((r-d)/iw)) + 0.5*(1.0+tanh((x-xr)/iw))'
  []
  [ic_func_gr0] # Top Right grain- Right section
    type = ParsedFunction
    symbol_names = 'iw x0 y0 r xr'
    symbol_values = '1000 10000 10000 5000 20000'
    expression = 'd:=sqrt((x-x0)^2+(y-y0)^2);
    0.5*(1.0-tanh((r-d)/iw)) * 0.5*(1.0-tanh((x-xr)/iw))'
  []
[]
...
[Materials]
  [consts]
    ...
    prop_names = 'Va negOverVa hgb' #cvieq_mask cvueq_mask csieq_mask csueq_mask'
    prop_values = '0.04092 -24.4379 0' #0.0 0.0 0.0 0.0'
  []
  [k_constants]
    type = GenericConstantMaterial

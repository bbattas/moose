##############################################################################
# File: 03_sub_noExternal_smallerPore_NMC_1step.i
# File Location: /examples/sintering/paper3/21_multiApps_IC/04_gb_lenticular_pore/00_sub
...
# -----
# Description:
#  Way smaller pore, decreasing IW to actually show it (from 2000)
#  r at 200nm for width of 0.4um, but that means iw needs to be down to 200ish
#  Shaved 1 OOM of pore r and iw here, so also trimmed the whole problem down 1
#   oom for DoF purposes (pore still centered in the same xy though)
##############################################################################
...
[Functions]
  [ic_func_pore] # Pore and exterior right
    ...
    symbol_values = '100 10000 10000 200'
  ...
  [ic_func_gr0] # Top grain
    ...
    symbol_values = '100 10000 10000 200'
    expression = 'd:=sqrt((x-x0)^2+(y-y0)^2);
    0.5*(1.0-tanh((r-d)/iw)) * 0.5*(1.0-tanh((y-y0)/iw))'
  []

##############################################################################
# File: 15_v_betterSize_FE.i
# File Location: /examples/sintering/paper3/21_multiApps_IC/04_gb_lenticular_pore/15_v_betterSize_FE
...
# -----
# Description:
#  small pore was basically he same as normal (4um r instead of 5um), this is actually
#   smaller, with pore r = 500nm. but IW is also smaller (400 isntead of 2000)
#   so the scaling is different and better already
#
##############################################################################
...
[MultiApps]
  [full_NMC]
    ...
    execute_on = initial
    positions = '0 0 0'
    input_files = ../00_sub/04_sub_betterSize_NMC_1step.i
  []
[]
...
[AuxVariables]
  ...
  [bnds]
  []
  [F_loc] # Combined bulk and gradiant energy, with bulk in a material
    order = CONSTANT
    family = MONOMIAL
  []
[]
...
[Materials]
  [consts]
    ...
    prop_names = 'Va negOverVa'
    prop_values = '0.04092 -24.4379'
  []
  [k_constants]
    type = GenericConstantMaterial
  ...
  [omegas_out]
    ...
    material_property_names = 'omegas omegav hs hv'
    expression = 'hs * omegas'
    outputs = 'exodus'
  []
  [omegav_out]
    type = ParsedMaterial
    ...
    material_property_names = 'omegas omegav hs hv'
    expression = 'hv * omegav'
    outputs = 'exodus'
  []
  [omega_out]
    type = ParsedMaterial
    property_name = omega_out
    material_property_names = 'omegas omegav hs hv'
    expression = 'hs * omegas + hv * omegav'
    outputs = 'exodus'
  []
  # Hemholtz parabolic free energy
  [fvac_v]
    type = ParsedMaterial
    property_name = fvac_v
    material_property_names = 'ksu kvu cvac cv_eq hv'
    expression = 'hv * 0.5 * kvu * (cvac - 1)^2'
    outputs = 'exodus'
  []
  [fvac_s]
    type = ParsedMaterial
    property_name = fvac_s
    material_property_names = 'ksu kvu cvac cv_eq hs'
    expression = 'hs * 0.5 * ksu * (cvac - cv_eq)^2'
    outputs = 'exodus'
  []
  [fvac_out]
    type = ParsedMaterial
    property_name = fvac_out
    material_property_names = 'ksu kvu cvac cv_eq hs hv'
    expression = 'hv * 0.5 * kvu * (cvac - 1)^2  +  hs * 0.5 * ksu * (cvac - cv_eq)^2'
    outputs = 'exodus'
  []
  # Bulk switch gradient stuff
  [f_bulk]
    type = ParsedMaterial
    ...
    expression = 'mu*(phi^4/4-phi^2/2 + gr0^4/4-gr0^2/2 + gr1^4/4-gr1^2/2
                  + gamma*(phi^2*(gr0^2+gr1^2) + gr0^2*(gr1^2)) + 0.25)'
    outputs = exodus
  []
  [f_switch]
    type = ParsedMaterial
  ...
  [GP_total]
    type = ParsedMaterial
    property_name = GP_total
    coupled_variables = 'F_loc'
    material_property_names = 'omega_out'
    expression = 'F_loc + omega_out'
    outputs = 'exodus'
  []
[]

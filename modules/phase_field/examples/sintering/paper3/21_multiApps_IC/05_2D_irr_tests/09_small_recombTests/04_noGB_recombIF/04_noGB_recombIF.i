##############################################################################
# File: 04_noGB_recombIF.i
# File Location: /examples/sintering/paper3/21_multiApps_IC/05_2D_irr_tests/09_small_recombTests/04_noGB_recombIF
...
# -----
# Description:
#  Using an if statemetn to hard disable recombnation on GB
#
#
#
##############################################################################
...
[Materials]
  ...
  [rho_recomb] #This one is off on GB?
    ...
    material_property_names = 'Va a_r(phi) combined_rho_vac(wvac,phi) combined_rho_int(wint,phi) hv hgb'
    expression = 'out:=a_r * combined_rho_vac * combined_rho_int * Va * (1-hgb);
                  if((out>0.0 & hv<=1e-6),if(hgb>1e-6,0.0,0.0-out),0.0)'
    outputs = nemesis #'nemesis'
  []
  [rho_mixing_vac]
    type = DerivativeParsedMaterial

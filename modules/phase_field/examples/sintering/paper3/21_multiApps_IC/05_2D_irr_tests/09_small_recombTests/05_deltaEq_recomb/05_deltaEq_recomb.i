##############################################################################
# File: 05_deltaEq_recomb.i
# File Location: /examples/sintering/paper3/21_multiApps_IC/05_2D_irr_tests/09_small_recombTests/05_deltaEq_recomb
...
# -----
# Description:
#  Testing using c-ceq in recombination instead of just c
#
#
#
##############################################################################
...
[Materials]
  ...
  [rho_recomb] #This one is off on GB?
    ...
    material_property_names = 'Va a_r(phi) combined_rho_vac(wvac,phi) combined_rho_int(wint,phi) hv cv_eq ci_eq'
    expression = 'delcv:=combined_rho_vac - (hv + (1-hv)*cv_eq)/Va;
                  delci:=combined_rho_int - ((1-hv)*ci_eq/Va);
                  out:=a_r * if(delcv>0.0,delcv,0.0) * if(delci>0.0,delci,0.0) * Va;

##############################################################################
# File: 06_theq_thresholds.i
# File Location: /examples/sintering/paper3/21_multiApps_IC/05_2D_irr_tests/09_small_recombTests/06_theq_thresholds
...
# -----
# Description:
#  Testing recombination stuff.  This is using lower limit thresholds of 1e-4
#   on basically everything in irradiation, lots of if statements
#  Using the thermal values for k and c though!
#
##############################################################################
...
[Materials]
  ...
  #   coupled_variables = 'phi gr0 gr1' # gr1 gr2'
  #   derivative_order = 2
  #   material_property_names = 'hs(phi) cv_eq(gr0,gr1)' #' cv_eq(gr0,gr1,gr2,gr3,gr4,gr5,gr6,gr7)'
  #   # constant_names = 'cb cgb'
  #   # constant_expressions = '3.877e-04 4.347e-03' #Irradiation
  #   # # constant_expressions = '2.424e-06 5.130e-03' #No Irradiation- LANL
  #   # expression = 'hg:=16 * ( (gr0 * gr1)^2 + (gr0 * gr2)^2 + (gr0 * gr3)^2 + (gr0 * gr4)^2 +
  ...
  #   coupled_variables = 'phi gr0 gr1' # gr1 gr2'
  #   derivative_order = 2
  #   material_property_names = 'hs(phi) ci_eq(gr0,gr1)' #' ci_eq(gr0,gr1,gr2,gr3,gr4,gr5,gr6,gr7)'
  #   # constant_names = 'cb cgb'
  #   # constant_expressions = '7.258e-09 5.900e-06' #Irradiation
  #   # # constant_expressions = '1.667e-32 6.170e-08' #'1.667e-32 6.170e-08' #No Irradiation- LANL
  #   # # constant_expressions = '2.424e-06 5.130e-03' #No Irradiation VACANCY- LANL
  ...
  [rho_recomb] #This one is off on GB?
    ...
    material_property_names = 'Va a_r(phi) combined_rho_vac(wvac,phi) combined_rho_int(wint,phi) hv(phi)'
    expression = 'out:=a_r * combined_rho_vac * combined_rho_int * Va;
                  if((out>0.0 & combined_rho_int>=0.0 & combined_rho_vac>=0.0 & hv<=1e-4),0.0-out,0.0)'
    outputs = nemesis #'nemesis'
  []
  [rho_mixing_vac]
    type = DerivativeParsedMaterial

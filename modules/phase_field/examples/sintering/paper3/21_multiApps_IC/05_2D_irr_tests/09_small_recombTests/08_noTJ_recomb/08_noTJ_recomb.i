##############################################################################
# File: 08_noTJ_recomb.i
# File Location: /examples/sintering/paper3/21_multiApps_IC/05_2D_irr_tests/09_small_recombTests/08_noTJ_recomb
...
# -----
# Description:
#  Disabling recombination on the triple junctions
#  This might help with the pores pinching vertically? but we still do want this
#   because we should see annihilation at triple junctions mroe than in gb really...
#
##############################################################################
...
[Materials]
  ...
  [htj]
    ...
    # expression = '16 * ( (gr0 * gr1)^2 )'
    expression = 'ht:=27 * ( (phi * gr0 * gr1) );
                  if(ht>1e-4,ht,0.0)' #+ + (gr0 * gr2)^2 + (gr1 * gr2)^2
    # expression = '4*(1 - (gr0^2 + gr1^2 + etad0^2))^2'
    # outputs = nemesis
  []
  [L_mat]
  ...
  [htj_out]
    type = ParsedMaterial
    property_name = htj_out
    material_property_names = 'htj'
    expression = 'htj'
    outputs = 'nemesis'
  []
  # IRRADIATION
  ...
  [rho_recomb] #This one is off on GB?
    ...
    expression = 'out:=a_r * combined_rho_vac * combined_rho_int * Va * (1-htj);

##############################################################################
# File: 01_2D_9grain_FR1e-8.i
# File Location: /examples/sintering/paper3/21_multiApps_IC/06_larger_irradiation/01_2D_9grain_FR1e-8
...
# -----
# Description:
#  Testing the full irradiation case with new amterials and whatnot using the
#   standard 1e-8 fission rate (fis/nm3s) (1e13/cm3s) on a 2D system with
#   9 grains (8um avg) and 8 pores (6% solid area)
#  1 node of 30 should about do it
##############################################################################
...
[MultiApps]
  [full_NMC]
    ...
    execute_on = initial
    positions = '0 0 0'
    input_files = ../00_sub/01_sub_2D_9grain.i
  []
[]

##############################################################################
# File: 16_long_shortSwitch0.1_noNegIf_genShort.i
# File Location: /examples/sintering/paper3/21_multiApps_IC/08_timestepping/16_long_shortSwitch0.1_noNegIf_genShort
...
# -----
# Description:
#  Input 06 longer/14  BUT with the generation set using the narrow hs instead of
#   just hs and a threshold cutoff
#
#
##############################################################################
...
[MultiApps]
  [full_NMC]
    ...
    execute_on = initial
    positions = '0 0 0'
    input_files = ../../06_larger_irradiation/00_sub/01_sub_2D_9grain.i
  []
[]
...
[Transfers]
  [from_NMC]
    ...
    source_variable = 'gr0 gr1 gr2 gr3 gr4 phi cvac_aux_ir cint_aux_ir'
...
[Materials]
  ...
  [c_gen]
    ...
    coupled_variables = 'phi'
    derivative_order = 1
    constant_names = 'Nc Nd noise f_dot p0'
    constant_expressions = '2 5 1 ${f_dot} 0.1'
    material_property_names = 'hs(phi) Va hv(phi)'
    expression = 'hss:=if(phi<=0.0,1.0,if(phi>=p0,0.0,1 - (6*(phi/p0)^5 - 15*(phi/p0)^4 + 10*(phi/p0)^3)));
                  f_dot * noise * Nc * Nd * hss * Va'

##############################################################################
# File: 12_hgb_test.i
# File Location: /examples/sintering/test_inputs/12_hgb_test
# Created Date: Saturday August 24th 2024
# Author: Brandon Battas (bbattas@ufl.edu)
# -----
# Last Modified: Saturday August 24th 2024
# Modified By: Brandon Battas
# -----
# Description:
#  Testing my various hgb options, including the new material i created for it
#
#
#
##############################################################################
...
[Materials]
  [hgb_old]
    ...
    property_name = hgb_old
    derivative_order = 2
    coupled_variables = 'gr0 gr1 gr2 gr3 gr4 gr5 gr6 gr7'
    expression = '4*(1 - (gr0^2 + gr1^2 + gr2^2 + gr3^2 + gr4^2 + gr5^2 + gr6^2 + gr7^2 ))^2'
    outputs = none
  []
  [hgb_current]
    type = DerivativeParsedMaterial
    property_name = hgb_current
    derivative_order = 2
    coupled_variables = 'gr0 gr1 gr2 gr3 gr4 gr5 gr6 gr7'
    expression = '16 * ( (gr0 * gr1)^2 + (gr0 * gr2)^2 + (gr0 * gr3)^2 + (gr0 * gr4)^2 +
  ...
  [cv_eq]
    ...
    expression = 'hg:=4*(1 - (gr0^2 + gr1^2 + gr2^2 + gr3^2 + gr4^2 + gr5^2 + gr6^2 + gr7^2 ))^2;
  ...
  [cint]
    ...
    outputs = none
  []
  # HS Times HGBs to see if its including pores or not
  [hs_hgb_old]
    type = ParsedMaterial
    property_name = hs_hgb_old
    material_property_names = 'hs hgb_old'
    expression = 'hs * hgb_old'
    outputs = nemesis
  []
  [hs_hgb_current]
    type = ParsedMaterial
    property_name = hs_hgb_current
    material_property_names = 'hs hgb_current'
    expression = 'hs * hgb_current'
    outputs = nemesis
  []
  [hs_hgb_new]
    type = ParsedMaterial
    property_name = hs_hgb_new
    material_property_names = 'hs hgb_new'
    expression = 'hs * hgb_new'
    outputs = nemesis
  []
  # Derivative check
  [hgb_current_dgr1]
    type = ParsedMaterial
    property_name = hgb_current_dgr1
    material_property_names = 'dHGB:=D[hgb_current(gr1),gr1]'
    expression = 'dHGB'
    outputs = 'nemesis'
  []
  [hgb_new_dgr1]
    type = ParsedMaterial
    property_name = hgb_new_dgr1
    material_property_names = 'dHGB:=D[hgb_new(gr1),gr1]'
    expression = 'dHGB'
    outputs = 'nemesis'
  []
  # SecondDerivative check
  [hgb_current_dgr1gr2]
    type = ParsedMaterial
    property_name = hgb_current_dgr1gr2
    material_property_names = 'dHGB:=D[hgb_current(gr1,gr2),gr1,gr2]'
    expression = 'dHGB'
    outputs = 'nemesis'
  []
  [hgb_new_dgr1gr2]
    type = ParsedMaterial
    property_name = hgb_new_dgr1gr2
    material_property_names = 'dHGB:=D[hgb_new(gr1,gr2),gr1,gr2]'
    expression = 'dHGB'
    outputs = 'nemesis'
  []
[]

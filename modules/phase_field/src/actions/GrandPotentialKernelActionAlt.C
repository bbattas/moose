...
#include "GrandPotentialKernelActionAlt.h"
#include "Factory.h"
#include "Parser.h"
#include "Conversion.h"
#include "FEProblem.h"

registerMooseAction("PhaseFieldApp", GrandPotentialKernelActionAlt, "add_kernel");

InputParameters
GrandPotentialKernelActionAlt::validParams()
{
  ...
  parameters.addClassDescription(
      ...
      "using different masscons kernels for each phase");
...
GrandPotentialKernelActionAlt::GrandPotentialKernelActionAlt(const InputParameters & parameters)
  : Action(parameters)
{
}

void
GrandPotentialKernelActionAlt::act()
{
  ...
  if (mass_conservation) // mass conservation kernels with conc and chempot coupling
  {
    ...
    for (unsigned int i = 0; i < n_w; ++i)
    {
      ...
      for (unsigned int j = i * n_hj; j < (i + 1) * n_hj; ++j)
      {
        // MatReaction
        ...
        params.set<bool>("implicit") = implicity;
        params.set<bool>("use_displaced_mesh") = displaced_mesh;
        kernel_name = "MBD_" + w_names[i] + "_" + hj_c_min[j]; // all_etas[j]

...
GBInclination::validParams()
{
  ...
  params.addClassDescription("Define inclination dependent properties for AGG.");
  ...
  params.addParam<bool>("aniso_L",
                        false,
                        "Is L fully anisotropic (including dL/dinc), requires 2nd order mesh and "
                        "variables, else L anisotropic but no derivatives.");
  ...
  params.addParam<bool>("stiffness", true, "Include the stiffness (d2gamma/dgradeta2), else = 0.");
  params.addParam<bool>(
      "well_props",
      false,
